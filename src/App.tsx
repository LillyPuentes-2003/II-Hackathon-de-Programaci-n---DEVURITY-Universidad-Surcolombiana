...
import CertificacionIA from './components/Certificacion';

const MOCK_REQUESTS: HelpRequest[] = [
  { id: 'REQ-001', userId: 'u1', userName: 'María García', type: 'alimentos', description: 'Necesitamos víveres para 3 familias en situación de calle.', location: 'Centro, Neiva', lat: 2.9273, lng: -75.2819, urgency: 'alta', peopleAffected: 12, status: 'pendiente', timestamp: Date.now() - 1000 * 60 * 30 },
  { id: 'REQ-002', userId: 'u2', userName: 'Juan Pérez', type: 'salud', description: 'Se requiere insulina para adulto mayor. Farmacias cerradas en la zona.', location: 'Barrio Miraflores, Neiva', lat: 2.9350, lng: -75.2900, urgency: 'alta', peopleAffected: 1, status: 'asignada', assignedTo: 'v1', assignedName: 'Carlos Voluntario', timestamp: Date.now() - 1000 * 60 * 120 },
  { id: 'REQ-003', userId: 'u3', userName: 'Elena R.', type: 'transporte', description: 'Traslado urgente a centro médico para control prenatal.', location: 'Barrio Quirinal, Neiva', lat: 2.9200, lng: -75.2750, urgency: 'media', peopleAffected: 2, status: 'en-camino', assignedTo: 'v2', assignedName: 'Ana Transp', timestamp: Date.now() - 1000 * 60 * 45 },
  { id: 'REQ-004', userId: 'u4', userName: 'Comedor Social', type: 'alimentos', description: 'Falta de gas para cocinar almuerzos de hoy.', location: 'Barrio Calixto, Neiva', lat: 2.9100, lng: -75.2700, urgency: 'alta', peopleAffected: 50, status: 'pendiente', timestamp: Date.now() - 1000 * 60 * 10 },
  { id: 'REQ-005', userId: 'u5', userName: 'Pedro S.', type: 'refugio', description: 'Familia desalojada busca refugio temporal por 2 noches.', location: 'Barrio Timanco, Neiva', lat: 2.9400, lng: -75.2600, urgency: 'media', peopleAffected: 4, status: 'en-revision', timestamp: Date.now() - 1000 * 60 * 300 },
];

const HELP_TYPE_ICONS: Record<HelpType, any> = {
  alimentos: Utensils, salud: Stethoscope, transporte: Truck, refugio: Home, otros: MoreHorizontal,
};
...
const Badge = ({ children, className }: { children: React.ReactNode, className?: string }) => (
  <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border ${className}`}>{children}</span>
);

const Card = ({ children, className, onClick }: { children: React.ReactNode, className?: string, onClick?: () => void }) => (
  <div onClick={onClick} className={`bg-card-bg rounded-xl border border-border shadow-sm overflow-hidden ${onClick ? 'cursor-pointer hover:border-primary/30 transition-all' : ''} ${className || ''}`}>
    {children}
  </div>
);
...
export default function App() {
  ...
  const simulateHelp = (requestId: string) => {
    ...
    setTimeout(() => {
      setRequests(prev => prev.map(req => req.id === requestId ? { ...req, status: 'asignada', assignedTo: 'me', assignedName: 'Tú (Voluntario)' } : req));
  ...
  const SplashView = () => (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-primary flex flex-col items-center justify-center z-[100]">
      <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: 'spring', damping: 12 }} className="text-center space-y-6">
        ...
        <div className="flex gap-1 justify-center">
          {[0, 1, 2].map(i => (
            <motion.div key={i} animate={{ opacity: [0.3, 1, 0.3] }} transition={{ duration: 1, repeat: Infinity, delay: i * 0.2 }} className="w-2 h-2 bg-white rounded-full" />
          ))}
        </div>
      </motion.div>
    </motion.div>
  ...
  const LandingView = () => (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-10 py-10">
      <div className="text-center space-y-6 max-w-2xl mx-auto">
        ...
        <p className="text-lg text-text-muted">La red solidaria en tiempo real que transforma la intención en acción directa.</p>
        <div className="flex flex-col sm:flex-row gap-4 justify-center pt-6">
          <button onClick={() => handleNavigate('register')} className="px-8 py-4 bg-primary text-white rounded-2xl font-bold text-lg shadow-xl shadow-primary/20 hover:scale-[1.02] transition-all">Empezar ahora</button>
          <button onClick={() => handleNavigate('live-map')} className="px-8 py-4 bg-white text-primary border-2 border-primary/10 rounded-2xl font-bold text-lg hover:bg-slate-50 transition-all flex items-center justify-center gap-2">
            <MapIcon className="w-5 h-5" /> Ver mapa en vivo
          </button>
        </div>
      </div>
      ...
      <Card className="p-8">
        ...
        <form className="space-y-6" onSubmit={(e) => { e.preventDefault(); handleNavigate('landing'); }}>
          ...
          <div className="space-y-2">
            ...
            <input required type="tel" className="w-full px-4 py-3 rounded-xl border border-border bg-slate-50 outline-none focus:border-primary transition-all" placeholder="+57 300 000 0000" />
          </div>
          <div className="space-y-2">
          ...
          <button type="submit" className="w-full py-4 bg-primary text-white rounded-xl font-bold text-lg shadow-lg shadow-primary/10">Continuar</button>
        </form>
      </Card>
    </motion.div>
  );
        ...
        <form className="grid grid-cols-1 md:grid-cols-2 gap-6" onSubmit={(e) => { e.preventDefault(); handleNavigate('request-confirmation'); }}>
            ...
            <div className="flex gap-2">
              {(['baja', 'media', 'alta'] as UrgencyLevel[]).map(lvl => (
                <label key={lvl} className="flex-1">
                  ...
                  <div className="py-3 text-center rounded-xl border border-border text-[10px] font-bold uppercase tracking-widest cursor-pointer peer-checked:border-primary peer-checked:bg-primary/5 peer-checked:text-primary transition-all">{lvl}</div>
                </label>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <input type="text" className="w-full pl-10 pr-4 py-3 rounded-xl border border-border bg-slate-50 outline-none" placeholder="Ej. Barrio Miraflores, Neiva" />
          ...
          <div className="md:col-span-2 flex flex-col sm:flex-row gap-4 pt-4">
            ...
            <button type="submit" className="flex-[2] py-4 bg-primary text-white rounded-xl font-bold text-lg shadow-lg shadow-primary/10">Enviar solicitud</button>
          </div>
        </form>
      </Card>
    </motion.div>
  ...
  const ConfirmationView = () => (
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="max-w-md mx-auto py-16 text-center space-y-8">
      ...
      <div className="flex flex-col gap-3">
        <button onClick={() => handleNavigate('request-status')} className="w-full py-4 bg-primary text-white rounded-xl font-bold shadow-lg shadow-primary/10">Ver estado de mi solicitud</button>
        <button onClick={() => handleNavigate('live-map')} className="w-full py-4 border border-border text-text-main rounded-xl font-bold hover:bg-slate-50">Ver en el mapa</button>
      </div>
    </motion.div>
  );
        ...
        <div className="flex justify-between items-start mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-text-muted">Registrada el 15 Abr, 2026</p>
            </div>
          </div>
      ...
      <Card className="p-8">
        ...
        <form className="grid grid-cols-1 md:grid-cols-2 gap-6" onSubmit={(e) => { e.preventDefault(); handleNavigate('nearby-requests'); }}>
          ...
          <div className="md:col-span-2 space-y-2">
            ...
            <div className="relative">
              <input type="text" className="w-full pl-10 pr-4 py-3 rounded-xl border border-border bg-slate-50 outline-none" placeholder="Ej. Centro, Neiva" />
              <MapPin className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-text-muted" />
            </div>
          </div>
          <div className="md:col-span-2 space-y-2">
          ...
          <button type="submit" className="md:col-span-2 py-4 bg-secondary text-white rounded-xl font-bold text-lg shadow-lg shadow-secondary/10">Publicar ayuda</button>
        </form>
      </Card>
    </motion.div>
  );
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 bg-white p-1 rounded-xl border border-border shadow-sm overflow-x-auto scrollbar-hide">
          {['todos', 'alimentos', 'salud', 'transporte'].map(t => (
            <button key={t} onClick={() => setFilter(t as any)} className={`px-4 py-2 rounded-lg text-xs font-bold transition-all capitalize ${filter === t ? 'bg-primary text-white shadow-md' : 'text-text-muted hover:bg-slate-50'}`}>{t}</button>
          ))}
        </div>
      </div>
  ...
  const AssignmentConfirmationView = () => (
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="max-w-md mx-auto py-16 text-center space-y-8">
      ...
      <div className="space-y-2">
        ...
        <p className="text-text-muted">Has aceptado ayudar. Los datos de contacto ya están disponibles.</p>
      </div>
      <Card className="p-6 text-left space-y-4">
        ...
        <p className="text-xl font-bold">+57 312 345 6789</p>
  ...
  const LiveMapView = ({ requests, onHelp }: { requests: HelpRequest[], onHelp: (id: string) => void }) => {
    ...
    const userLoc = { lat: 2.9273, lng: -75.2819 };
    ...
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 top-[73px] z-0 flex flex-col md:flex-row bg-bg">
        <aside className="hidden md:flex flex-col w-80 lg:w-96 border-r border-border bg-white z-10 shadow-xl">
          <div className="p-6 border-b border-border space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-xl font-black tracking-tight flex items-center gap-2"><Activity className="w-5 h-5 text-primary" /> Dashboard</h3>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 scrollbar-hide bg-slate-50/30">
            ...
            {filteredBySearch.map((req) => (
              <div key={req.id}>
                <Card onClick={() => setSelectedRequest(req)} className={`p-4 transition-all border-l-4 group ${selectedRequest?.id === req.id ? 'border-l-primary bg-primary/5 ring-1 ring-primary/10' : 'border-l-transparent hover:bg-slate-50'}`}>
                  <div className="flex justify-between items-start mb-2">
                    <Badge className={URGENCY_COLORS[req.urgency]}>{req.urgency}</Badge>
                    <span className="text-[10px] font-bold text-text-muted uppercase tracking-widest flex items-center gap-1"><Clock className="w-3 h-3" /> 30m</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-slate-100">
                    <span className="text-[10px] font-bold text-text-muted flex items-center gap-1"><MapPin className="w-3 h-3 text-primary" /> {req.location}</span>
                    <span className="text-[10px] font-bold text-primary bg-primary/5 px-2 py-0.5 rounded-full">1.2 km</span>
                  </div>
                </Card>
              </div>
            ))}
          ...
          <div className="p-4 bg-white border-t border-border">
            <button onClick={() => handleNavigate('need-help-form')} className="w-full py-4 bg-primary text-white rounded-2xl font-bold text-sm shadow-lg shadow-primary/20 flex items-center justify-center gap-2 hover:scale-[1.02] transition-all">
              <AlertCircle className="w-5 h-5" /> Publicar Emergencia
            </button>
          </div>
        </aside>
        ...
        <div className="flex-1 relative bg-slate-100 overflow-hidden">
          <div className="absolute inset-0 pointer-events-none overflow-hidden">
            <div className="absolute inset-0 opacity-30" style={{ backgroundImage: `radial-gradient(#94a3b8 1px, transparent 1px)`, backgroundSize: '32px 32px' }} />
            ...
            {selectedRequest && (
              <svg className="absolute inset-0 w-full h-full pointer-events-none z-10">
                <motion.path
                  ...
                  d={`M ${(userLoc.lng + 76) * 10}% ${(6 - userLoc.lat) * 10}% L ${(selectedRequest.lng + 76) * 10}% ${(6 - selectedRequest.lat) * 10}%`}
                  stroke="#2563eb" strokeWidth="3" strokeDasharray="8 8" fill="none"
                />
              </svg>
            )}
          </AnimatePresence>

          <div className="absolute z-30 pointer-events-none" style={{ left: `${(userLoc.lng + 76) * 10}%`, top: `${(6 - userLoc.lat) * 10}%` }}>
            <div className="relative">
          ...
          {filteredBySearch.map((req, i) => (
            <motion.div key={req.id} initial={{ scale: 0, y: 20 }} animate={{ scale: 1, y: 0 }} transition={{ type: 'spring', damping: 12, delay: i * 0.05 }}
              className="absolute cursor-pointer z-20"
              style={{ left: `${(req.lng + 76) * 10}%`, top: `${(6 - req.lat) * 10}%` }}
              onClick={() => setSelectedRequest(req)}
            >
              <div className="relative group">
                {req.urgency === 'alta' && <div className="absolute inset-0 bg-urgency-high/30 rounded-full animate-ping scale-150" />}
                <div className={`w-10 h-10 rounded-2xl border-4 border-white shadow-2xl flex items-center justify-center transition-all group-hover:scale-110 group-hover:-translate-y-1 ${selectedRequest?.id === req.id ? 'ring-4 ring-primary/30 scale-110 -translate-y-1' : ''} ${req.urgency === 'alta' ? 'bg-urgency-high' : req.urgency === 'media' ? 'bg-urgency-medium' : 'bg-urgency-low'}`}>
                  {React.createElement(HELP_TYPE_ICONS[req.type], { className: "w-5 h-5 text-white" })}
                </div>
                <div className={`absolute bottom-full left-1/2 -translate-x-1/2 mb-3 transition-all ${selectedRequest?.id === req.id ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0'}`}>
          ...
          <div className="absolute top-6 left-6 right-6 flex flex-col sm:flex-row gap-4 z-30">
            <div className="flex-1 max-w-md relative group">
              ...
              <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-12 pr-4 py-4 bg-transparent rounded-2xl outline-none font-bold text-text-main placeholder:text-text-muted/60" placeholder="Buscar por nombre, zona o necesidad..." />
            ...
            <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
              <div className="flex bg-white/80 backdrop-blur-xl p-1.5 rounded-2xl shadow-2xl border border-white/40">
                {['todos', 'alimentos', 'salud', 'transporte', 'refugio'].map(t => (
                  <button key={t} onClick={() => setFilter(t as any)} className={`px-5 py-2.5 rounded-xl text-xs font-black transition-all capitalize whitespace-nowrap ${filter === t ? 'bg-primary text-white shadow-lg' : 'text-text-muted hover:bg-white/40'}`}>{t}</button>
                ))}
              </div>
            </div>
          </div>
          ...
          <div className="absolute top-24 right-6 z-30 hidden lg:block">
            <button onClick={() => setShowLegend(!showLegend)} className="bg-white/80 backdrop-blur-md p-3 rounded-xl shadow-xl border border-white/40 flex items-center gap-2 text-xs font-bold text-text-main">
              <Info className="w-4 h-4 text-primary" /> Leyenda
            </button>
            <AnimatePresence>
              {showLegend && (
                <motion.div initial={{ opacity: 0, scale: 0.9, x: 20 }} animate={{ opacity: 1, scale: 1, x: 0 }} exit={{ opacity: 0, scale: 0.9, x: 20 }} className="mt-2 bg-white/90 backdrop-blur-xl p-4 rounded-2xl shadow-2xl border border-white/40 space-y-3 w-48">
                  {[['bg-urgency-high', 'Urgencia Alta'], ['bg-urgency-medium', 'Urgencia Media'], ['bg-urgency-low', 'Urgencia Baja'], ['bg-primary', 'Tu Ubicación']].map(([color, label]) => (
          ...
          <div className="absolute bottom-8 right-8 flex flex-col gap-4 z-30">
            <button className="p-4 bg-white rounded-2xl shadow-2xl border border-border text-text-main hover:scale-110 transition-all">
              <Navigation className="w-6 h-6" />
            </button>
            <button onClick={() => handleNavigate('need-help-form')} className="p-4 bg-primary text-white rounded-2xl shadow-2xl shadow-primary/40 hover:scale-110 transition-all">
          ...
          <AnimatePresence>
            {selectedRequest && (
              <motion.div initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 100, opacity: 0 }} className="absolute bottom-8 left-8 right-8 md:left-auto md:w-[400px] z-40">
                <Card className="p-6 relative shadow-2xl border-2 border-primary/10 bg-white/95 backdrop-blur-md">
                  <button onClick={() => setSelectedRequest(null)} className="absolute top-4 right-4 p-2 text-text-muted hover:text-text-main hover:bg-slate-100 rounded-full transition-all">
                  ...
                  <div className="flex items-start gap-4 mb-6">
                    ...
                    <div className="pt-1">
                      ...
                      <p className="text-xs text-text-muted font-bold flex items-center gap-1"><MapPin className="w-3 h-3 text-primary" /> {selectedRequest.location} • 1.2 km de ti</p>
                    </div>
                  </div>
                  <div className="bg-slate-50/80 rounded-2xl p-4 mb-6 border border-border/50">
                    <p className="text-sm text-text-main leading-relaxed font-medium">"{selectedRequest.description}"</p>
                  </div>
                  <div className="flex gap-3">
                    {selectedRequest.status === 'pendiente' ? (
                      <button onClick={() => onHelp(selectedRequest.id)} disabled={isSimulating} className="flex-1 py-4 bg-primary text-white rounded-2xl font-bold text-sm shadow-lg shadow-primary/20 flex items-center justify-center gap-2 disabled:opacity-70">
                        {isSimulating ? (<><motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 1, ease: "linear" }}><Activity className="w-4 h-4" /></motion.div>Procesando...</>) : <>Aceptar y ayudar</>}
                      </button>
                    ) : (
                      <button onClick={() => handleNavigate('request-detail')} className="flex-1 py-4 bg-secondary text-white rounded-2xl font-bold text-sm shadow-lg shadow-secondary/20">Ver detalles de entrega</button>
                    )}
                    <button className="p-4 border border-border rounded-2xl text-text-muted hover:bg-slate-50 transition-all"><Phone className="w-5 h-5" /></button>
                  </div>
                </Card>
              </motion.div>
            )}
  ...
  const AdminPanelView = () => {
    const [adminTab, setAdminTab] = useState<'overview' | 'analytics' | 'validator' | 'certificacion'>('overview');
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8 py-8">
        ...
        <div className="flex gap-2 bg-slate-100 p-1 rounded-xl w-fit flex-wrap">
          {[
            ...
            { key: 'validator', label: '🤖 Validación IA' },
            { key: 'certificacion', label: '🎓 Certificación' },
          ].map((tab) => (
            <button
              key={tab.key}
              onClick={() => setAdminTab(tab.key as any)}
              className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${adminTab === tab.key ? 'bg-white shadow-sm text-primary' : 'text-slate-500 hover:text-slate-700'}`}
            >
              {tab.label}
            </button>
          ))}
        ...
        {adminTab === 'validator' && <AIValidator />}
        {adminTab === 'certificacion' && <CertificacionIA />}
        {adminTab === 'overview' && (
          <>
  ...
  return (
    <div className="min-h-screen bg-bg text-text-main font-sans selection:bg-primary/10 selection:text-primary">
      ...
      {view !== 'splash' && (
        <header className="sticky top-0 z-50 bg-card-bg border-b border-border px-6 py-4">
          <div className="max-w-6xl mx-auto flex items-center justify-between">
            <button onClick={() => handleNavigate('landing')} className="flex items-center gap-2 group transition-all">
            ...
            <div className="flex items-center gap-4">
              ...
              {view !== 'landing' ? (
                <button onClick={() => handleNavigate('landing')} className="text-sm font-semibold text-text-muted hover:text-text-main flex items-center gap-1 transition-colors">
      ...
      <AnimatePresence>
        {notification && (
          <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 50, opacity: 0 }} className="fixed bottom-24 left-1/2 -translate-x-1/2 z-[100] w-full max-w-sm px-6">
            <div className={`p-4 rounded-2xl shadow-2xl border flex items-center gap-3 ${notification.type === 'success' ? 'bg-emerald-500 text-white border-emerald-400' : 'bg-red-500 text-white border-red-400'}`}>
